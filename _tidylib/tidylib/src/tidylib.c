...
char * tidyGetTextNodeText2(TidyDoc tdoc,TidyNode tnod){
    TidyBuffer buf;
    tidyBufInit(&buf);
    tidyNodeGetText(tdoc, tnod, &buf);
    return buf.bp?(char *)buf.bp:"";
}
void tidydumpNode2( TidyDoc tdoc,TidyNode tnod, int indent )
{
  TidyDocImpl * tim;
  TidyNode child;
  ...
  for ( child = tidyGetChild(tnod); child; child = tidyGetNext(child) )
  {
    ...
    if (tidyNodeGetType(child)==TidyNode_Text){
      ...
      char *text =tidyGetTextNodeText2(tdoc,child);
      printf("\n\t year:%s",text);
      tidyFreePtr(text);
    ...
    tidydumpNode2(tdoc, child, indent + 4 );
  }
}
uint tidyGetNodeStart(TidyNode tnod){
    Node * node;
    node = tidyNodeToImpl(tnod);
    uint size=0;
    size=node->start;
...
uint tidyGetNodeEnd(TidyNode tnod){
    Node * node;
    node = tidyNodeToImpl(tnod);
    uint size=0;
    size=node->end;
...
char * tidyGetLexer(TidyDoc tdoc){
    TidyDocImpl * tim;
    tim = tidyDocToImpl(tdoc); 
    return (char * )tim->lexer->lexbuf;
}
uint tidyGetStrlen(char * lex){
    return strlen(lex);
}
uint tidyGetSizeof(char * lex){
    return sizeof(lex);
}
char * tidyGetTextNodeText(TidyDoc tdoc,TidyNode tnod){
    TidyDocImpl * tim;
    tim = tidyDocToImpl(tdoc); 
    int size=0;
    Node * node;
